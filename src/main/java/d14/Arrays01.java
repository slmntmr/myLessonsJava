...
public class Arrays01 {
    public static void main(String[] args) {
        ...
        System.out.println("---------kendı yaptıgım-----------");
     OptionalInt max= Arrays.stream(ages).max();
        System.out.println("max = " + max);

     OptionalInt   min=Arrays.stream(ages).min();
        System.out.println("min = " + min);

        int sumerNumber=Arrays.stream(ages).sum();
        System.out.println("sumer = " + sumerNumber);

        System.out.println("maxMınSummer: "+max+min);
